export function paginate(arr, limit = 50, cursor) {
    ...
    const idx = arr.findIndex((x) => x.id === cursor);
    if (idx <= 0)
        return { items: [], nextCursor: undefined, total: arr.length };
    const start = Math.max(0, idx - limit);
    return {
        items: arr.slice(start, idx),
        nextCursor: start > 0 ? arr[start - 1].id : undefined,
        total: arr.length,
    };
}
